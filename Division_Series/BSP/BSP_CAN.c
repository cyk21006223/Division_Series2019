...
void CAN1_QuickInit(void)
{
	...
	NVIC_QuickInit(CAN1_RX0_IRQn, 3, 0);
...
void CAN2_QuickInit(void)
{
	...
	NVIC_QuickInit(CAN2_RX0_IRQn, 3, 0);
	/* FIFO0 message pending interrupt */
...
/**
  ...
  * @retval None
  */
void CAN_SendData(CAN_TypeDef* CANx, \
	                uint8_t id_type, \
                  uint32_t id, \
                  uint8_t data[8])
{	  
	...
	TxMessage.DLC = 0x08;						 //数据长度(最大为8字节)
	...
	CanTx_mailbox = CAN_Transmit(CANx, &TxMessage);
}
/**
  * @brief  检验CAN发送是否发送成功
  * @retval 0失败 其他成功
...
{
  return CAN_TransmitStatus(CANx, CanTx_mailbox);
}
